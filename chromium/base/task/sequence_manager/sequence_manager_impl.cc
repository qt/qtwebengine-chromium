...
std::atomic<TimeDelta> g_task_leeway_SMI{WakeUp::kDefaultLeeway};
...
void SequenceManagerImpl::InitializeFeatures() {
  ...
  g_task_leeway_SMI.store(kTaskLeewayParam.Get(), std::memory_order_relaxed);
}
...
void SequenceManagerImpl::MaybeAddLeewayToTask(Task& task) const {
  if (!main_thread_only().time_domain)
    task.leeway = g_task_leeway_SMI.load(std::memory_order_relaxed);
}

...
class MaglevFrameTranslationBuilder {
 ...
 private:
  ...
  // Emit a captured object instead of a literal
  void BuildCapturedHeapNumber(const VirtualObject* object) {
    DCHECK_EQ(object->type(), VirtualObject::kHeapNumber);
    DCHECK(object->map().IsHeapNumberMap());
  ...
  void BuildVirtualObject(const VirtualObject* object,
    ...
    switch (object->type()) {
      case VirtualObject::kHeapNumber:
        return BuildCapturedHeapNumber(object);

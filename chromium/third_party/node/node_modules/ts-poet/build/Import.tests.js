...
describe("Import", () => {
    it("parsing implicitly defined (non-imported) symbols", () => {
        const parsed = Import_1.Import.from("Some.Symbol.Depth");
    ...
    it("parsing named import: exported symbol explicit, source relative to current dir", () => {
        const parsed = Import_1.Import.from("BackendService@./some/local/source/file");
        expect(parsed).toBeInstanceOf(Import_1.ImportsName);
        const sym = parsed;
    ...
    it("parsing named import: exported symbol explicit, source is implied module", () => {
        const parsed = Import_1.Import.from("SomeOtherSymbolDepth@rxjs/Observable");
        expect(parsed).toBeInstanceOf(Import_1.ImportsName);
        const sym = parsed;
    ...
    it("parsing named import: exported symbol explicit, source is implied namespaced module", () => {
        const parsed = Import_1.Import.from("SomeOtherSymbolDepth@@lexical/react");
        expect(parsed).toBeInstanceOf(Import_1.ImportsName);
        const sym = parsed;
        expect(sym.symbol).toEqual("SomeOtherSymbolDepth");
    ...
    it("parsing named import: type import", () => {
        const parsed = Import_1.Import.from("t:Observable@rxjs/Observable");
        expect(parsed).toBeInstanceOf(Import_1.ImportsName);
        const sym = parsed;
    ...
    it("parsing all import: exported symbol explicit, source is implied module", () => {
        const parsed = Import_1.Import.from("SomeOther*rxjs/Observable");
        expect(parsed).toBeInstanceOf(Import_1.ImportsAll);
        const sym = parsed;
    ...
    it("parsing default import", () => {
        const parsed = Import_1.Import.from("DataLoader=dataloader");
        expect(parsed).toBeInstanceOf(Import_1.ImportsDefault);
        const sym = parsed;
    ...
    it("parsing side effect import: exported symbol made available as side effect of import", () => {
        const parsed = Import_1.Import.from("describe+mocha");
        expect(parsed).toBeInstanceOf(Import_1.SideEffect);
        const sym = parsed;
    ...
    it("can handle relative imports", () => {
        expect((0, Import_1.maybeRelativePath)("zaz/Zaz", "./foo/Foo")).toEqual("../foo/Foo");
    });
    it("can interpret importMappings", () => {
        const parsed = Import_1.Import.from("Empty@./google/protobuf/empty");
        expect(parsed).toBeInstanceOf(Import_1.ImportsName);
        const sym = parsed;
    ...
    it("can handle relative imports where importing file is in a folder with the same name as file it is importing in a folder above it", () => {
        expect((0, Import_1.maybeRelativePath)("./foo/bar", "./foo")).toEqual("../foo");
    });
    it("import an export with a more convenient alias", () => {
        const parsed = Import_1.Import.from("Observable:CustomizedObservable@packages/override-properties/Observable");
        expect(parsed).toBeInstanceOf(Import_1.ImportsName);
        const sym = parsed;
        expect(sym.symbol).toEqual("CustomizedObservable");
        expect(sym.source).toEqual("packages/override-properties/Observable");
        expect(emit(sym)).toMatchInlineSnapshot(`"import { Observable as CustomizedObservable } from 'packages/override-properties/Observable';"`);
        // type import
        const typeImportParsed = Import_1.Import.from("t:Observable:CustomizedObservable@packages/override-properties/Observable");
        expect(typeImportParsed).toBeInstanceOf(Import_1.ImportsName);
        const typeSym = typeImportParsed;
        expect(typeSym.symbol).toEqual("CustomizedObservable");
        expect(typeSym.source).toEqual("packages/override-properties/Observable");
        expect(emit(typeSym)).toMatchInlineSnapshot(`"import { type Observable as CustomizedObservable } from 'packages/override-properties/Observable';"`);
    });
    it("can handle module spec with square brackets", () => {
        const parsed = Import_1.Import.from("handler@./route/[name]/index");
        expect(parsed).toBeInstanceOf(Import_1.ImportsName);
        const sym = parsed;
    ...
    it("can handle module spec with parenthesis", () => {
        const parsed = Import_1.Import.from("handler@./route/(group)/index");
        expect(parsed).toBeInstanceOf(Import_1.ImportsName);
        const sym = parsed;
        expect(sym.symbol).toEqual("handler");
    ...
    it("can handle module spec with unusual characters", () => {
        const parsed = Import_1.Import.from("handler@./route{/:name+}?/:path*");
        expect(parsed).toBeInstanceOf(Import_1.ImportsName);
        const sym = parsed;
        expect(sym.symbol).toEqual("handler");
    ...
    it("can handle a http URL module spec", () => {
        const parsed = Import_1.Import.from("something@http://example.com/something");
        expect(parsed).toBeInstanceOf(Import_1.ImportsName);
        const sym = parsed;
    ...
    it("can handle a node: module spec", () => {
        const parsed = Import_1.Import.from("URL@node:url");
        expect(parsed).toBeInstanceOf(Import_1.ImportsName);
        const sym = parsed;
    ...
    it("can handle a jsr: module spec", () => {
        const parsed = Import_1.Import.from("assertEquals@jsr:/@std/assert@^0.223.0/assert-equals");
        expect(parsed).toBeInstanceOf(Import_1.ImportsName);
        const sym = parsed;
        expect(sym.symbol).toEqual("assertEquals");
        expect(sym.source).toEqual("jsr:/@std/assert@^0.223.0/assert-equals");
        expect(emit(sym)).toMatchInlineSnapshot(`"import { assertEquals } from 'jsr:/@std/assert@^0.223.0/assert-equals';"`);
    });
    function emit(spec, importMappings = {}, forceRequireImports = [], importExtensions = true) {
        return (0, Import_1.emitImports)([spec], "", importMappings, forceRequireImports, importExtensions).trim();
    }
});

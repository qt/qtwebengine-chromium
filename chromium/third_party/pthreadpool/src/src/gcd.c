...
struct pthreadpool* pthreadpool_create(size_t threads_count) {
  if (threads_count == 0) {
    ...
    if (sysctlbyname("hw.logicalcpu_max", &threads, &sizeof_threads, NULL, 0) !=
        0) {
      return NULL;
    }
...
PTHREADPOOL_INTERNAL void pthreadpool_parallelize(
  ...
  dispatch_semaphore_wait(threadpool->execution_semaphore,
  ...
  dispatch_apply_f(threads_count.value, DISPATCH_APPLY_AUTO, threadpool,
                   thread_main);

                                              ...
                                              const ImageDefinitionMtl &imageDef)
{
    ...
    if (unpackBuffer)
    {
        ...
        else
        {
            ...
            mtl::BlitCommandEncoder *blitEncoder =
                GetBlitCommandEncoderForResources(contextMtl, {sourceBuffer.get(), imageDef.image.get()});
            CopyBufferToOriginalTextureIfDstIsAView(
                                                        ...
                                                        const ImageDefinitionMtl &imageDef)
{
    ...
    if (unpackBuffer)
    {
        ...
        BufferMtl *unpackBufferMtl = mtl::GetImpl(unpackBuffer);
        if (!imageFormat.getCaps().writable || imageFormat.hasDepthOrStencilBits() ||
            ...
            for (NSUInteger d = 0; d < mtlArea.size.depth; ++d)
            {
                ...
                for (NSUInteger r = 0; r < mtlArea.size.height; ++r)
                {
                    ...
                    ANGLE_TRY(UploadTextureContents(
                        ...
                        conversionRow.data(), dstRowPitch, 0, kAvoidStagingBuffers, imageDef.image));
                }
            }
        }  // if (imageFormat.intendedAngleFormat().isBlock)
    }  // if (unpackBuffer)

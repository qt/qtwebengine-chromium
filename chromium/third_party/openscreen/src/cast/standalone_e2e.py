...
def _get_build_paths():
  ...
  root_path = pathlib.Path(
      os.environ[ROOT_ENVVAR]
      if os.getenv(ROOT_ENVVAR)
      else subprocess.getoutput('git rev-parse --show-toplevel')
  )
  assert root_path.exists(), 'Could not find openscreen root!'

  build_path = (
      pathlib.Path(os.environ[BUILD_ENVVAR])
      if os.getenv(BUILD_ENVVAR)
      else root_path.joinpath('out', 'Default').resolve()
  )
  assert build_path.exists(), 'Could not find openscreen build!'

  BuildPaths = namedtuple(
      'BuildPaths', 'root build test_video cast_receiver cast_sender'
  )
  return BuildPaths(
      root=root_path,
      build=build_path,
...
class StandaloneCastTest(unittest.TestCase):
  ...
  def download_video(cls):
    ...
    with request.urlopen(
        TEST_VIDEO_URL, context=ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
    ) as url:
  ...
  def get_output(self, flags, codec=None):
    ...
    output = (
        receiver_process.communicate(timeout=PROCESS_TIMEOUT)[1].decode(
            'utf-8'
        ),
        sender_process.communicate(timeout=PROCESS_TIMEOUT)[1].decode('utf-8'),
    )
    ...
    if not TestFlags.USE_REMOTING in flags:
      self.assertEqual(
          sender_process.returncode, 0, 'sender had non-zero exit code'
      )
    return output
  ...
  @unittest.skipUnless(
      os.getenv(LIBAOM_ENVVAR), 'Skipping AV1 test since LibAOM not installed.'
  )

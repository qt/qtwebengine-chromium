...
download the pre-built clang coverage tools, which are not downloaded by
default.  The easiest way to do this is to set a custom variable in your
`.gclient` file. Under the "openscreen" solution, add:
...
manually
([googleapis link](https://storage.googleapis.com/chromium-browser-clang-staging/)).

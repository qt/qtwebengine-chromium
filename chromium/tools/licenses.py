...
def ParseDir(path, root, require_license_file=True, optional_keys=None):
  ...
  if path in SPECIAL_CASES:
    ...
    for line in io.open(readme_path, encoding="utf-8"):
      line = line.strip()
      if not line:
        break
...
def ProcessAdditionalReadmePathsJson(root, dirname, third_party_dirs):
  ...
  if os.path.exists(additional_paths_file):
    with io.open(additional_paths_file, encoding="utf-8") as paths_file:
...
def GenerateCredits(
  ...
  def MetadataToTemplateEntry(metadata, entry_template):
    env = {
        ...
        'url': metadata['URL'],
        'license': io.open(metadata['License File'], encoding="utf-8").read(),
  ...
  entry_template = io.open(entry_template_file, encoding="utf-8").read()
  entries = []
  ...
  file_template = io.open(file_template_file, encoding="utf-8").read()
  ...
  if output_file:
    changed = True
    try:
      old_output = io.open(output_file, 'r', encoding="utf-8").read()

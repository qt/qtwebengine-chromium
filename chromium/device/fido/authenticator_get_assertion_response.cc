...
std::vector<uint8_t> GetSerializedCtapDeviceResponse(
  ...
  response_map.emplace(cbor::CBORValue(3), cbor::CBORValue(response.signature()));

  if (response.user_entity()) {
    response_map.emplace(cbor::CBORValue(4), PublicKeyCredentialUserEntity::ConvertToCBOR(
                                *response.user_entity()));
  }
  if (response.num_credentials()) {
    response_map.emplace(cbor::CBORValue(5), response.num_credentials().value());
  }
